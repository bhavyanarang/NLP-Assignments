...
def decode(inp, HMM, smooth=True, k=1):
    ...
    for j in range(1, T):
        ...
        for i in range(Q):
            trellis[i, j] = np.max([trellis[l, j - 1] * (
                        (A[mapping[l]][mapping[i]] + k) / (sum(A[mapping[l]].values()) + k * Q)) * (
                                                (B[mapping[i]][inp[j]] + k) / (sum(B[mapping[i]].values()) + Q * k))
                                    if
                                    inp[j] in B[mapping[i]]
                                    else trellis[l, j - 1] * (
                        (A[mapping[l]][mapping[i]] + k) / (sum(A[mapping[l]].values()) + k * Q)) * (
                                                                             (B[mapping[i]]['UNK'] + k) / (
                                                                                 sum(B[mapping[i]].values()) + Q * k))
                                    for l in range(Q)])
            bt[i, j] = np.argmax([trellis[l, j - 1] * (
                        (A[mapping[l]][mapping[i]] + k) / (sum(A[mapping[l]].values()) + k * Q)) * (
                                              (B[mapping[i]][inp[j]] + k) / (sum(B[mapping[i]].values()) + Q * k)) if
                                  inp[j] in B[mapping[i]] else trellis[l, j - 1] * (
                        (A[mapping[l]][mapping[i]] + k) / (sum(A[mapping[l]].values()) + k * Q)) * (
                                                                           (B[mapping[i]]['UNK'] + k) / (
                                                                               sum(B[mapping[i]].values()) + Q * k)) for
                                  l in range(Q)])

    path = list()
    maxi = np.argmax(
        [trellis[l, T - 1] * ((A[mapping[l]]['.'] + k) / (sum(A[mapping[l]].values()) + k * Q)) for l in range(Q)])
    path.append(maxi)
    i = int(maxi)
...
def main():
    ...
    parser.add_argument("-u", "--unk", help="Threshold for replacing emissions with 'UNK'. Default is 1", default=1,
                        type=int)
    parser.add_argument("-k", help="k value for add-k smoothening. Default is 1", default=1, type=int)
    parser.add_argument("-s", "--save",
                        help="Path where to save trained model. If set to Y then a file, 'HMM.pkl' is created in the current directory.",
                        default=0, choices=[0, 1], type=int)
    parser.add_argument("-o", "--out",
                        help="Where to output the result. Provide a filepath. By default, the output is displayed at stdout.",
